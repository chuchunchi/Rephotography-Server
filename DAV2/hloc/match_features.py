...
import matchers
from utils.read_write_model import read_model, getp3d
...
confs = {
    ...
    'superglue3d': {
        'output': 'matches-superglue3d',
        'model': {
            'name': 'superglue3d',
            'weights' : 'model_statedict_epoch_51',
...
def main(conf: Dict, pairs: Path, features: Union[Path, str],
         ...
         Zd3d : bool = False, sfm : Path = None, name2ref : Dict = None):
    ...
    match_from_paths(
        conf, pairs, matches, features_q, features_ref, exhaustive = exhaustive, Zd3d=Zd3d, sfm=sfm, name2ref=name2ref)
...
def match_from_paths(conf: Dict, pairs_path: Path, match_path: Path,
                     ...
                     name2ref: Dict = None, exhaustive: bool = False,
                     Zd3d : bool = False, sfm : Path = None):
    ...
    if Zd3d:
        _, db_images, points3D = read_model(str(sfm))
    for (name0, name1) in tqdm(pairs, smoothing=.1):
        ...
        if not Zd3d :
            ...
            p3d, valid = getp3d(name1, db_images, points3D)
            if p3d is None:
                continue
            data['keypoints1'] = torch.from_numpy(p3d[...,0:2].__array__()).float().to(device)
            data['scores1'] = torch.from_numpy(p3d[...,2].__array__()).float().to(device)
            with h5py.File(str(feature_paths_refs[name2ref[name1]]), 'r') as fd:
                grp = fd[name1]
                desc = np.transpose(grp['descriptors'].__array__())
                desc = np.transpose(desc[valid])
                data['descriptors1'] = torch.from_numpy(desc).float().to(device)
                data['image1'] = torch.empty((1,)+tuple(grp['image_size'])[::-1])
