...
def main(descriptors, output, num_matched,
         ...
         db_prefix=None, db_list=None, db_model=None, db_descriptors=None, db_names=None):
    ...
    if not isinstance(db_descriptors, list):
        ...
        db_desc = torch.from_numpy(np.stack(db_descriptors, 0)).to(device).float()
