...
results = outputs / f'Aachen_hloc_superpoint+superglue3d_netvlad{args.num_loc}.txt'
...
matcher_conf = match_features.confs['superglue3d']
...
'''loc_matches = match_features.main(
    matcher_conf, loc_pairs, feature_conf['output'], outputs, Zd3d=True, sfm=reference_sfm)'''

localize_sfm.main(
    reference_sfm,
    dataset / 'queries/*_time_queries_with_intrinsics.txt',
    loc_pairs,
    Path(outputs, feature_conf['output']+'.h5'),
    Path(outputs, f"{feature_conf['output']}_{matcher_conf['output']}_{loc_pairs.stem}.h5"), # loc_matches,
    results,
    covisibility_clustering=False,
    Zd3d=True)  # not required with SuperPoint+SuperGlue
