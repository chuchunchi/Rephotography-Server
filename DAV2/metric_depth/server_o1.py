"""
移动方向、距离全部由orbslam计算，kadfp只负责视觉对齐误差反馈
"""
...
class Server:
        ...
        while True:
            ...
            try:
                ...
                if args.kadfp:
                    ...
                    kadfp_error = self.get_kadfp_error(curr_img,target_img)
                    cost2 = time.time() - start_time2
                    print(f"flow estimate 執行時間：{cost2} 秒") 
                    print(f"total cost time: {cost1+cost2} 秒")

                    self.conn.sendall(struct.pack('!I', self.video_idx))
                    self.conn.sendall(struct.pack('!f', kadfp_error))
                    # 接收客户端回传merge_state和对齐标志
                    ...
                    if self.video_idx<video_len-1: # 帧序号必须小于影片总帧数
                        if (self.compare_counter>=10 or client_align_flag == 1) and (merge_state): # 比较次数达到10次或者对齐成功
    ...
    def get_kadfp_error(self,curr_img,target_img):
        flow = estimator.estimate(curr_img, target_img)
        kadfp_error = flow_to_error(curr_img, target_img, flow, self.video_idx,self.curr_idx, KADFP_results_path)
        return kadfp_error
