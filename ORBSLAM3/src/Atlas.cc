...
vector<KeyFrame *> Atlas::GetAllKeyframesByMap(long unsigned int id){
    try{
        unique_lock<mutex> lock(mMutexAtlas);
        for(Map* pMap_i : mspMaps){
            // cout<<"Map id: "<<pMap_i->GetId()<<endl;
            if(pMap_i->GetId() == id){
                return pMap_i->GetAllKeyFrames();;
            }
        }
    }catch(exception e){
        cout<<"Error in GetAllKeyframesByMap: "<<e.what()<<endl;
    }
    return vector<KeyFrame *>();
}

vector<KeyFrame *> Atlas::GetMergedAllKeyframes(){
    try{
        unique_lock<mutex> lock(mMutexAtlas);
        return mpCurrentMap->GetAllKeyFrames();
    }catch(exception e){
