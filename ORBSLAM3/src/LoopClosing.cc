...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC):
    ...
    mbLoopDetected(false), mbMergeState(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC)
{
    // 连续性阈值
            ...
            if(bFindedRegion)
            {
                ...
                if(mbMergeDetected)
                {
                    ...
                    else
                    {
                        ...
                        cout<<"mSold_new: "<<phi(0)<<" "<<phi(1)<<" "<<phi(2)<<endl;
...
void LoopClosing::setMergeState(bool state){
    mbMergeState = state;
}
