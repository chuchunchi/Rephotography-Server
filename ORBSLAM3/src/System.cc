    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    //if(false) // TODO
    {
        ...
        mptViewer = new thread(&Viewer::Run, mpViewer, std::ref(KeyBoardCommand));
...
void System::LocalizationMode(){
    mpViewer->setLocalizationMode();
}
...
bool System::getMergeState(){
    return mpLoopCloser->getMergeState();
}

std::string System::TrackingStateToString(int state) {
    switch (state) {
        case -1:
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
    // cout<<"[system] 此帧的跟踪位姿状态： "<<TrackingStateToString(mTrackingState)<<endl;
    return Tcw;
}
...
std::vector<KeyFrame *> System::GetAllKeyFramesFromAtlas()
{   
    try{
        return mpAtlas->GetAllKeyframesByMap(0);
    }catch(exception e){
...
std::vector<KeyFrame *> System::GetAllKeyFramesFromMergedMap()
{
    try{
        return mpAtlas->GetMergedAllKeyframes();
    }catch(exception e){
        cout << "Exception occurred: " << e.what() << endl;
        return std::vector<KeyFrame*>();
    }
