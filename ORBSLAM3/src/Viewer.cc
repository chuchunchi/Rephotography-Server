...
Viewer::Viewer(System* pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking, const string &strSettingPath, Settings* settings):
    ...
    mbFinishRequested(false), mbFinished(true), mbStopped(true), mbStopRequested(false),menuLocalizationModeState(false)
{
    if(settings){
...
void Viewer::Run(int& command)
{
    ...
    while(1)
    {
        ...
        if(menuLocalizationModeState){
            menuLocalizationMode = menuLocalizationModeState;
        }
...
void Viewer::setLocalizationMode() {
    menuLocalizationModeState = !menuLocalizationModeState;
}
